...
function boxBase:Update()
    ...
    if ESP.Highlighted == self.Object then
       color = ESP.HighlightColor
        ...
        color = self.Color or self.ColorDynamic and self:ColorDynamic() or  ESP.Color
    end
    ...
    if ESP.Names then
        ...
        if Vis5 then
            ...
            self.Components.Distance.Text = string.format("[%s][%s]", math.floor((cam.CFrame.Position - cf.Position).Magnitude), math.floor(self.Humanoid.Health).."/"..self.Humanoid.MaxHealth)
...
function ESP:Add(obj, options)
    ...
    local Position = plrs:GetPlayerFromCharacter(obj)
...
local function HumanoidAdded(humanoid)
	if not humanoid:IsA("Humanoid") or plrs:FindFirstChild(humanoid.Parent.Name) == nil or humanoid.Parent == plr.Character then return end
