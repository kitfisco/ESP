...
function boxBase:Update()
    ...
    if Vis5 then
        ...
        if self.Extra.Team then
            self.Components.Distance.Text = string.format("[%s][%s][%s]", math.floor((Camera.CFrame.Position - cf.Position).Magnitude), math.floor(self.Extra.Humanoid.Health).."/"..math.floor(self.Extra.Humanoid.MaxHealth), self.Extra.Team.Value)
        else
            ...
            if character ~= LocalPlayer.Character then
                if character:FindFirstChild("HumanoidRootPart") then
                    ESP:Add(character, {
                        ...
                        Extra = {
                            Humanoid = humanoid,
                            Team = character:WaitForChild("Team", 5)
                        }
                    })
                else
